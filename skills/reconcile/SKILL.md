...
This phase is **automated** -- scan the codebase, don't ask about what already exists.
...
Draft SPEC.md and present a summary:
...
If corrections are needed, update and re-present.
...
Something changed in the user's understanding of the product. This is the most conversational mode.
...
5. Present the diff: "Here's what changed in VISION.md -- [summary]."
...
5. Update the header (last reconciled date, drift status -> CURRENT).
...
1. **NEVER hand the user a doc full of brackets.** Interview them. They talk, you write.
...
7. **Don't touch BUILDING.md.** That document is updated as part of the release process, not during reconciliation.
...
9. **Use the project's existing voice.** Read BUILDING.md and VISION.md before writing to match tone.
